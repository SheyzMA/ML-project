...
\addlinespace
LogReg ($\eta{=}0.3$, 500) & Acc.\,(\%) & \textbf{86.04} & \textbf{88.25} \\
KNN Cls.\ ($k{=}9$)   & Acc.\,(\%) & 80.63 & 79.75 \\
\addlinespace
LogReg ($\eta{=}0.3$, 500) & Macro-F1 & 0.727 & \textbf{0.819} \\
...
\noindent \textbf{Linear Regression} is fastest overall: the pseudo-inverse is a single LAPACK call ($<$1\,ms) and prediction is one matrix multiply. \textbf{Logistic Regression} trains in $\sim$119\,ms (500 gradient steps on 1600 samples) but predicts in under 0.1\,ms once weights are stored. \textbf{KNN} has near-zero training time (it only stores data), but prediction costs $\sim$35\,ms — for each of the 400 test points, distances to all 1600 training points must be recomputed ($O(N \cdot D)$ per query), making it the slowest at inference.
...
\begin{itemize}[leftmargin=*]
    ...
    \item \textbf{Classification:} Logistic Regression is vastly superior to KNN, achieving 88.25\% test accuracy and a high Macro-F1 score (0.819). As illustrated by the confusion matrix (Figure~\ref{fig:cm}) and class distribution (Figure~\ref{fig:dist}), the model handles the dominant \emph{Low} class well, but two errors stand out: \emph{Medium}$\to$\emph{Low} (29/107, driven by class imbalance) and \emph{High}$\to$\emph{Medium} (5/13), where severe addiction cases are systematically downgraded — a clinically important failure mode.
\end{itemize}
