...
\begin{table}[H]
\centering
\caption{Final method comparison on validation and test sets. \textbf{Bold} marks the best test score per metric; \textcolor{DeepTeal!75}{shading} highlights the winning method per task.}
\label{tab:results}
...
Linear Regression        & MSE $\downarrow$ & 0.931 & \textbf{0.994} \\
KNN Reg.\ ($k{=}9$)     & MSE $\downarrow$ & 1.947 & 1.865 \\
\midrule
\multicolumn{4}{@{}l}{\textit{Classification}} \\
\rowcolor{DeepTeal!15}
LogReg ($\eta{=}0.3$, 500) & Acc.\,(\%) $\uparrow$ & 86.04 & \textbf{88.25} \\
KNN Cls.\ ($k{=}9$)      & Acc.\,(\%) $\uparrow$ & 80.63 & 79.75 \\
\rowcolor{DeepTeal!15}
LogReg ($\eta{=}0.3$, 500) & Macro-F1 $\uparrow$  & 0.727 & \textbf{0.819} \\
