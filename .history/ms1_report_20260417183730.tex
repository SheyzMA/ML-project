...
\caption{Confusion matrix for LogReg ($\eta{=}0.3$, 1000 iters) on the test set. Errors are mostly isolated to the \emph{Medium}/\emph{High} boundary.}
\label{fig:cm}
\end{figure}
...
\begin{itemize}[leftmargin=*]
    ...
    \item \textbf{Classification:} Logistic Regression is vastly superior to KNN, achieving 88.25\% test accuracy and a high Macro-F1 score (0.819). As illustrated by the confusion matrix (Figure~\ref{fig:cm}) and class distribution (Figure~\ref{fig:dist}), the model successfully distinguishes between the dominant classes, with minimal overlap occurring strictly at the harder \emph{Medium}/\emph{High} boundary.
\end{itemize}
